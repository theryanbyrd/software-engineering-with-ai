...
3. **Run `npm run verify` after every step.** Not at the end. After every step.
...
- **Extract type** — when the same shape appears in multiple places.
- **Inline temp** — when a variable is used once and the inline form is clearer.
- **Move file** — to align with the architecture.
